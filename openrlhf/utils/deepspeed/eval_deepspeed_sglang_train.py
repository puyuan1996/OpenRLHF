...
def create_sub_group(group_size):
    ...
    # 修改点：Gloo 后端不支持 ReduceOp.AVG，因此使用 SUM，
    # 然后手动除以组内的数量以获得平均数
    dist.all_reduce(tmp, group=group, op=dist.ReduceOp.SUM)
    tmp /= group_size  # 平均值计算
    dist.barrier()
...
def get_sglang_engine(args):
    # TODO: maybe we can get tp group directly from vllm, but this can test comm.
    ...
    print(f"sglang_tp_group:{sglang_tp_group}")

    return verl_engine, sglang_tp_group
